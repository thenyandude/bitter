...
function UserHome() {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      const response = await axios.get(`http://10.12.5.206/api/posts?userId=${userId}`);
  ...
  const handleCreatePost = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://10.12.5.206/api/posts', { userId, title, content });
